...
-- =========================================================
-- CTIMER Epic 3 Master customization
-- =========================================================
...
begin
  select id into v_timer_id from public.timers
  where code = upper(trim(p_code)) limit 1;
  if v_timer_id is null then raise exception 'Invalid code'; end if;
  if not public.verify_admin_token(v_timer_id, p_admin_token) then
    raise exception 'Invalid admin token';
  end if;
  ...
  update public.timers set name = v_name where id = v_timer_id;
end $$;
...
begin
  ...
  if p_asset_id is null then
    ...
    insert into public.timer_assets(
      ...
      v_timer_id, trim(p_url), coalesce(p_enabled, true), coalesce(p_sort_order, 0),
      nullif(trim(coalesce(p_sponsor_name, '')), ''),
      nullif(trim(coalesce(p_sponsor_tier, '')), ''), coalesce(p_weight, 1)
    ) returning id into v_asset_id;
    return v_asset_id;
  end if;
  ...
  where id = p_asset_id and timer_id = v_timer_id
  returning id into v_asset_id;
  if v_asset_id is null then raise exception 'Invalid asset'; end if;
  return v_asset_id;
end $$;
